...
- **Seleção top‑k** – escolhe os `top_k` (ex.: 1) melhores experts entre os candidatos e calcula os pesos via softmax. A combinação das saídas é feita de forma **vetorizada**: por cada expert `e`, uma máscara `(top_k_indices == e).float()` multiplicada pelos pesos e somada ao longo da dimensão top‑k devolve o peso total desse expert por token (`weight_e`, shape `(B, T)`); os tokens com `weight_e > 0` são recolhidos com `nonzero()` (gather), passam pelo expert (um único forward em lote) e o resultado ponderado é devolvido à posição original com `final_out[idx] += weight_e[idx].unsqueeze(-1) * out` (scatter‑add). Equivalente ao loop explícito token‑a‑token, mas sem o loop aninhado.
